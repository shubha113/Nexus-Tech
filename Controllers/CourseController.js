...
export const getAllCourses = catchAsyncError(async (req, res, next) => {
    ...
    const courses = await Course.find({title:{$regex:keyword, $options:"i"}, category:{$regex:category, $options:"i"},}).select("-lectures");
    res.status(200).json({
        success: true,
        courses,
    });
...
export const getCourseLectures = catchAsyncError(async (req, res, next) => {
  ...
  //max video size 100mb coz we are using free cloudinary plan
...
export const deleteLecture = catchAsyncError(async (req, res, next) => {
    ...
    course.numOfVideos = course.lecture.length;
    await course.save();
    res.status(201).json({
        success: true,
